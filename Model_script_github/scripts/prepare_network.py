...
if __name__ == "__main__":
    ...
    for o in opts:
        ...
        if oo[0] == 'vre': #in case of VRE
            carrier = ['solar','onwind'] #wildcard snippet i.e. 'solar' in our case
            attr_lookup = {"mc": "marginal_cost"} #creates lookup table with key:value string
            attr = attr_lookup[oo[1][0:2]] #oo[1] equals the second string i.e. "c1.5", oo[1][0] checks the first letter i.e. "c" a looks for the key at the key value string "attr_lookup"
            factor = float(oo[1][2:]) #takes the string "c1.5" and position it to "1.5"
            comps = {"Generator", "Link", "StorageUnit", "Store"} #the list which is to be iterated
            for c in n.iterate_components(comps): #iterate_components is a pypsa function and reveal all the data per component -> PyPSA/pypsa/components.py line 948 ... It returns also a list "c"
                sel = c.df.carrier.str.contains(carrier[0]) #carrier = solar, sets "sel" to a boolean list that identify for all carrier if "solar" string matches n.carrier string & makes it "True" if it is the case
                c.df.loc[sel,attr] = factor  ###Only looks at buses that are true (solar=True) and multiplies attribute "capital_cost" of solar plant by factor
                #print(c.df.loc[sel,attr])
                sel = c.df.carrier.str.contains(carrier[1])
                c.df.loc[sel,attr] = factor  ###Only looks at buses that are true (solar=True) and multiplies attribute "capital_cost" of solar plant by factor
                print(c.df.loc[sel,attr])
        
        elif oo[0] == 'discharger': #creates boolean. Operate only if wildcard (i.e. 'solar') is in suptechs/n.carriers.index
            carrier = ['battery discharger','fuel cell'] #wildcard snippet i.e. 'solar' in our case
            attr_lookup = {"mc": "marginal_cost"} #creates lookup table with key:value string
            attr = attr_lookup[oo[1][0:2]] #oo[1] equals the second string i.e. "c1.5", oo[1][0] checks the first letter i.e. "c" a looks for the key at the key value string "attr_lookup"
            factor = float(oo[1][2:]) #takes the string "c1.5" and position it to "1.5"
            comps = {"Generator", "Link", "StorageUnit", "Store"} #the list which is to be iterated
            ...
            carrier = ['solar','onwind','battery discharger','fuel cell'] #wildcard snippet i.e. 'solar' in our case
            attr_lookup = {"mc": "marginal_cost"} #creates lookup table with key:value string
            attr = attr_lookup[oo[2][0:2]] #oo[1] equals the second string i.e. "c1.5", oo[1][0] checks the first letter i.e. "c" a looks for the key at the key value string "attr_lookup"
            ratio = float(oo[1])
            mcbase = float(oo[2][6:]) #takes the string "c1.5" and position it to "1.5"
            comps = {"Generator", "Link", "StorageUnit", "Store"} #the list which is to be iterated
            for c in n.iterate_components(comps): #iterate_components is a pypsa function and reveal all the data per component -> PyPSA/pypsa/components.py line 948 ... It returns also a list "c"
                sel = c.df.carrier.str.contains(carrier[0]) #carrier = solar, sets "sel" to a boolean list that identify for all carrier if "solar" string matches n.carrier string & makes it "True" if it is the case
                c.df.loc[sel,attr] = mcbase  ###Only looks at buses that are true (solar=True) and multiplies attribute "capital_cost" of solar plant by factor
                sel = c.df.carrier.str.contains(carrier[1])
                c.df.loc[sel,attr] = mcbase  ###Only looks at buses that are true (solar=True) and multiplies attribute "capital_cost" of solar plant by factor
                #print(c.df.loc[sel,attr])
            for c in n.iterate_components(comps): #iterate_components is a pypsa function and reveal all the data per component -> PyPSA/pypsa/components.py line 948 ... It returns also a list "c"
                sel = c.df.carrier.str.contains(carrier[2]) #carrier = solar, sets "sel" to a boolean list that identify for all carrier if "solar" string matches n.carrier string & makes it "True" if it is the case
                c.df.loc[sel,attr] = ratio/mcbase  ###Only looks at buses that are true (solar=True) and multiplies attribute "capital_cost" of solar plant by factor
                sel = c.df.carrier.str.contains(carrier[3])
                c.df.loc[sel,attr] = ratio/mcbase  ###Only looks at buses that are true (solar=True) and multiplies attribute "capital_cost" of solar plant by factor
                #print(c.df.loc[sel,attr])
